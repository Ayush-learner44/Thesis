# traffic_v2 — Live testbed with the "real" tools
...
```bash
sudo apt install -y apache2-utils   # provides `ab`; scapy already installed
```
...
3. **Server on h0** (mininet): `xterm h0` → `python3 /home/ayush/my2/server.py`
...
```
python3 /home/ayush/my2/traffic_v2/attack_rate.py --flood            # max rate
python3 /home/ayush/my2/traffic_v2/attack_rate.py --rate 100         # 100 pps
python3 /home/ayush/my2/traffic_v2/attack_rate.py --count 500 --rate 200
python3 /home/ayush/my2/traffic_v2/attack_rate.py --duration 3       # 3s then stop
python3 /home/ayush/my2/traffic_v2/attack_rate.py --flood --spoof    # spoofed src (evades)
python3 /home/ayush/my2/traffic_v2/attack_rate.py --rate 5 --rand-port   # demo split-dilution
```
...
```
Puts h1..h8 at 1 / 2 / 5 / 10 / 25 / 50 / 100 / 250 pps, each sending 40
half-open SYNs (fixed port). **Do NOT use verify.py for this** — verify.py's
menu is hardwired to the old scripts. Just read the controller's `Blocked`
list against the ladder the sweep prints:
