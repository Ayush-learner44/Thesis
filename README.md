...
- **Partial-completion (stealth) floods evade** — the detector is completion-based, so it only blocks a *pure* half-open flood. A `completion_test.py` sweep (0/10/…/100%) shows only the 0% host is blocked; completing as little as **10%** of handshakes evades. This is fundamental, not a tuning gap: a 10%-completion attacker measures completion ratio ~0.30, *higher* than a legitimately RTT-lagged benign client (~0.06–0.14), so no completion threshold separates them without new false positives, and retraining would just trade FNs for FPs. Closing it needs an orthogonal signal (short-window SYN volume or destination-side entropy), out of scope for a completion-based detector.
